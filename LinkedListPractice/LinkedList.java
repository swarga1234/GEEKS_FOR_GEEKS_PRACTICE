...
public class LinkedList {
    ...
    public Node insertAtPos(int pos, int value){
        ...
        // If temp becomes null then surely we don't have the pos to insert. Like a linkedlist has 3 nodes and you are trying to insert at 6th position. So the insertion should fail
        if(temp==null){
            return head;
        }
    ...
    */
    public Node addTwoLists(Node num1, Node num2) {
        ...
        // Lets use the extra space first. We will create a res linkedlist to store the sum of num1 and num2. First we will reverse both the lists and then add them and again reverse the res.
        ...
        int carry=0;
        Node curr=res;
        // So we add the nos node by node. This addition has to stop when both the linkedlists have been traversed and there is no carry.
    ...
    public Node sortedInsert(int val){
        // Insert a new value in to a already sorted linked list such that the linked list remains sortedafter the insertion
    ...
    public int getMidNdode(Node head){
        // For getting the middle node of a linked list we can maintain 2 ptrs slow and fast. Fast will move twice the speed of slow. If fast reaches the end of the linked list then slow will be at the middle of the linkedlist
    ...
    public Node reverseRec1(Node head){
        ...
        // This solution is based on the principal that lets say we have nodes from 1 to n. like x1,x2,x3......,xn. Now if we reverse the linked list from x2 to xn and then reverse the link of node x1 we have a reversed linked list
    ...
    public Node reverseK(Node head, int k){
        ...
        while(curr!=null){
            ...
            // In the first iteration we will reverse the first k nodes.So the kth node should be the head after the whole linked list is reversed this way.
            if(firstPass){
                head=prev;
                firstPass=false;
            }
    ...
    public void removeLoop(Node head){
        ...
        // Now as a loop is detected so we need to remove it now. So the way to do is we will reset the slow_p to head. Now we will move the slow_p and fast_p by each time untill they meet. So mathematically when they meet, they will meet at the start of the cycle. Thus if we stop our while loop at the condition when slow_p.next==fast_p.next, then we can do fast_p.next=null to break the loop. The only time this trick fails is when the starting node of the cycle is head/ its a circular linked list. In that case we need to explicitly move the fast ptr to the end of the linked list and change its next to null.
        if(fast_p==null){
        ...
        //Now as we have handled the case for when the cycle starts at head.. we can now focus on the general logic.
        slow_p=head;
    ...
    /*
        ...
        The relative order of the odd and even nodes in the orginal list should be maintained in the output list. Also the output list should have the even nodes first followed by the odd nodes
    */
    public Node segregate(Node head){
        ...
        //Now we will traverse each node and check if the value is even / odd. If even then for the first even node we will mark it as es and ee. Same for the odd node. The subsecuent even nodes wil be added to the ee and the odd nodes to oe. After the whole list is proccessed as we need even nodes followed by odd nodes we will attach os to to ee.
        ...
        // Once all the nodes are traversed then we need to join the os with ee.next. But there may be scenario where there are no odd nos in the linked list or no even nos in the linked list. Then our original list is the answer. It should be unaltered.
        if(os==null || es==null){
            return head;
        }
        ee.next=os;
    ...
    public int findIntersectionPoint(Node head1, Node head2){
        ...
        // The head1 and head2 forms a Y shaped linked list which in like Y both the linked lists intersect at a point. How to find that? Lets count the no of nodes in head1 and head2 linked lists. Now take the abs(count1-count2). Traverse the first abs(count1-count2) nodes of the longer list, that would give both the list a level playing field. Now traverse both of them simultaneously. Anywhere the two nodes are equal is a intersection point.
